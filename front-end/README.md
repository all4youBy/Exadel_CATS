...
class App extends Component {
  ...
  render() {
    return (
      <div>
        <button onClick={this.handleClick}>MainProject</button>
      </div>
    );
  }
}
...
This will make `moduleA.js` and all its unique dependencies as a separate chunk that only loads after the user clicks the 'MainProject' button.
